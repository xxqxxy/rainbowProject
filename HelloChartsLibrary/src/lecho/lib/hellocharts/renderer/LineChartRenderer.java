            ...
            if (checkIfShouldDrawPoints(line)) {
                ...
                for (PointValue pointValue : line.getValues()) {
                    ...
                    if (isInArea(rawValueX, rawValueY, touchX, touchY, pointRadius + touchToleranceMargin)) {
                    	System.out.println("点击了曲线图的点");
//                    	pwDialog(valueIndex, context,touchX,touchY);
...
/*    PopupWindow popupWindow = null;
    private void pwDialog(int i,Context context,float x,float y){
  		if(popupWindow == null){
    		initPopWindow(i,context);
  		}
    	if(!popupWindow.isShowing()){
    		View v = null;
    		popupWindow.showAtLocation(null, Gravity.CENTER, (int)x,(int)y);
    	}else{
  			popupWindow.dismiss();
  		}
  	}
  	public void initPopWindow(int i, Context mContext) {
  		LayoutInflater inflater = LayoutInflater.from(mContext);
  		final View v = inflater.inflate(R.layout.his_pop, null);
  		
  		System.out.println("value i = "+i);
  		((TextView)v.findViewById(R.id.tv_his)).setText(i+"");
